...
# "$scriptDir/pdfsizeopt.sh" "$ghostscriptTemp" "$websiteDir/databases.pdf"
# rm "$ghostscriptTemp"
echo "$(date +'%0Y-%0m-%0d %0R:%0S'): Compressing the book from '$currentDir/book.pdf' to '$websiteDir/databases.pdf' using pdfsizeopt."
"$scriptDir/pdfsizeopt.sh" "$currentDir/book.pdf" "$websiteDir/databases.pdf"
...
PART_C='</body></html>'
BASE_URL='https\:\/\/thomasweise\.github\.io\/databases\/'
echo "${PART_A}Databases${PART_B}$("$PYTHON_INTERPRETER" -m markdown -o html ./README.md)$PART_C" > "$websiteDir/index.html"
