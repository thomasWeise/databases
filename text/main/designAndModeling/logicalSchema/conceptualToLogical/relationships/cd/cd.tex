...
\listingSQLandOutput{CD_insert_error_1}{%
...
\listingSQLandOutput{CD_insert_error_2}{%
It is impossible to insert a row into table~\sqlil{c} that does not reference a row in table~\sqlil{d}.%
}{}%
%
...
In our conceptual model, each entity of type~\emph{Person} could be a~\emph{Faculty} member or not.
...
A three-table solution makes no sense here.
We \emph{know} that every entity of type~C \emph{must} be connected to one entity of type~D.
The solution for implementing this relationship pattern in a \pgls{rdb} is therefore similar to the two-table variant in the previous section.
...
This column thus has a \sqlil{REFERENCES} constraint to the foreign key, which we add at the end of the script \cref{lst:CD_tables} via~\sqlil{ALTER TABLE}\sqlIdx{ALTER!TABLE}\sqlIdx{TABLE!ALTER}.
...
Each of these rows must provide a proper and unique foreign key~\sqlil{fkdid} referencing the primary key value~\sqlil{did} in table~\sqlil{d}.
...
Relating entities of type~C to multiple entities of type~D is impossible, because the column~\sqlil{fkcid} in table~\sqlil{c} can only have one value.
Relating entities of type~D to multiple entities of type~C is impossible, because of the \sqlil{UNIQUE} constraint imposed on it the column~\sqlil{fkcid} in table~\sqlil{c}.
It is impossible to insert a row into table~\sqlil{c} that references a row in table~\sqlil{d} that is already referenced by another row in table~\sqlil{c}, as shown in \cref{lst:CD_insert_error_1}.
Neither can we insert a row into table~\sqlil{c} that does \emph{not} reference any row in table~\sqlil{d}, because of the \sqlil{NOT NULL} constraint, as illustrated by \cref{lst:CD_insert_error_2}.
