...
\end{definition}%
%
If we map a conceptual models, say given as \pgls{ERD}, to the relational data model, we must also map the relationship patterns.
...
\end{itemize}%
%
In most courses that I have on \pgls{db}, the relationship types are discussed in the conceptual levels.
Then, there is only a very approximate and incomplete explanation on how these can actually be realized.
Maybe something along the lines of~\inQuotes{For this type, you use two tables\dots}.
It is usually discussed how the multiplicity and modality can be actually enforced, especially for more complex constellations like~\crowsFoot{S}{MM}{T}{MM}.

Well, you are lucky.
We will now set out to find how \emph{each} of the ten binary conceptual relationship types that we discussed can be implemented in \sql.
