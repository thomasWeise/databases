...
Let us again begin by establishing the basic tables that we need.
Then we sort out how to enforce the referential integrity with proper constraints.
...
We also need a table~\sqlil{r} for the entities of type~R.
It gets the primary key~\sqlil{rid} and the example attribute~\sqlil{y}.
...
Like in \dref{sec:rm:op}, each pair~\sqlil{(fkqid, fkrid)} can appear only once in the table.
This is because two specific rows in tables~\sqlil{q} and~\sqlil{r} can, of course, be related only once to each other.
This is implemented via the constraint~\sqlil{PRIMARY KEY (fkqid, fkrid)}.

Compared to \crowsFoot{O}{OM}{P}{OM}, the problem that we need to solve is that we need to enforce that each row in table~\sqlil{q} \emph{must be} related to (at least) one row in table~\sqlil{r}.
Back in \dref{sec:rm:gh}, we faced a similar problem, i.e., \emph{\inQuotes{How can we make sure that each entity of type~G is connected to at least one entity of type~H?}}
...
We solved this by creating a constraint that enforced that the primary key of that row in table~\sqlil{h} together with the foreign key to the table~\sqlil{g} would need to be the same as the foreign key of the row in table~\sqlil{g} and its primary key.
...
We again store a \inQuotes{first R} foreign key to a row in~\sqlil{r} as column~\sqlil{fkrid} in table~\sqlil{q}.
Then we add the constraint~\sqlil{q_qid_fkrid_fk} to table~\sqlil{q}.
It enforces~\sqlil{FOREIGN KEY (qid, fkrid) REFERENCES relate_q_and_r (fkqid, fkrid)}.
...
When we do this, the new row in table~\sqlil{relate_q_and_r} must use the primary key value of the new row in table~\sqlil{q} as foreign key.
...
The new row in table~\sqlil{relate_o_and_p} gets the primary key value pointing to the new row in table~\sqlil{q}.
Additionally, it gets the primary key value of the row in table~\sqlil{r} that it references.
Both insertions are executed together as one single command and the referential integrity constraints are checked at its end.
And it fits.%
\end{sloppypar}%
%
