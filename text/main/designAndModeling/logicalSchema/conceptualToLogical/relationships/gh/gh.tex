...
We will consider this attribute~\sqlil{fkhid} to identify the \inQuotes{first~H}~entity to which the row in table~\sqlil{g} is related.
...
Since each entity of type~H can be related to either zero or one entity of type~G, we add a column~\sqlil{fkgid} to table~\sqlil{h}.
The values in this column can be \sqlil{NULL}, in which case the corresponding row in~\sqlil{h} is not related to any entity of type~G.
If it is not \sqlil{NULL}, then it must be a proper foreign key reference to a row in table~\sqlil{g}.
The values in this column do \emph{not} need to be \sqlil{UNIQUE}, as multiple entities of type~H can be related to the very same entity of type~G.%
%
\begin{sloppypar}%
...
\end{sloppypar}%
%
We can enforce this in a somewhat weird way:
By creating the foreign key \sqlil{REFERENCES} constraint not on the single column~\sqlil{hid}, but by enforcing that the pair~\sqlil{(fkhid, gid)} from table~\sqlil{g} must also appear as pair~\sqlil{(hid, fkgid)} in table~\sqlil{h}.
...
The second element of the pair, i.e.,~\sqlil{gid}~(or, looking from the other side,~\sqlil{fkgid}) thus forces that this single row in table~\sqlil{h} has the fitting value~\sqlil{gid} stored in~\sqlil{fkgid}.
...
On the side of the table~\sqlil{g}, this constraint looks at a row and takes the value of its foreign key~\sqlil{fkhid} to table~\sqlil{h} together with its own primary key~\sqlil{gid} as a tuple~\sqlil{(fkhi, gid)}.
...
It is enforced that the pairs \sqlil{(fkhi, gid) == (hid, fkgid)} always fit.
\end{sloppypar}%
%
...
Inserting a row into table~\sqlil{g} requires us to specify a value for the foreign key~\sqlil{fkhid} and the corresponding row in table~\sqlil{h} must exist.
...
But could we insert a row into table~\sqlil{g} that references a row in table~\sqlil{h} that is not already related to an entity of type~G?
...
We cannot insert a row that does not reference an existing row in table~\sqlil{h}.
We cannot insert a row that references a row in table~\sqlil{h} which is not already connected to another row in table~\sqlil{g}.
...
We have created our two tables and protected their referential integrity using fierce constraints.
And these constraints need to be exactly like that, because we \emph{want} to implement \crowsFoot{G}{O1}{H}{MM}
Now we need to see that we can insert data into the tables.
...
However, when we insert an entity of type~G into our table~\sqlil{g}, we must, at the same time, create a relationship to an entity of type~H in table~\sqlil{h}.
This sounds impossible, but it is not.
We do this without problem by knowing that:%
%
...
%
In \postgresql~(and probably several other \pglspl{dbms}), referential integrity constraints are checked at the end of the execution of a transaction.
A transaction can group several commands together, that either all succeed or all fail~(in which case, the \db\ will not be modified).
A single command in \postgresql\ also is a transactions~\cite{PGDG:T}.
This means that referencial integrity constraints are checked at at the end of the command execution.
The changes are committed to the \db\ if the constraints are met and rolled back otherwise.

Now, to insert a row into table~\sqlil{g}, we also need to modify a record from table~\sqlil{h} that currently is unrelated to any row in table~\sqlil{g}.
We need to relate that row in table~\sqlil{h} to that new row in table~\sqlil{g}.
If we can wrap the insertion and the modification into a single \sql\ command, then things might be easy.
Otherwise we would need to learn how to explicitly use transactions, which would also be OK.

In order to modify the \sqlil{fkgid} value of a row in table~\sqlil{h}, we must know the primary key~\sqlil{gid} of the new row in table~\sqlil{g}.
...
It is evaluated only once and can be used like a read-only table in other parts of the query.
...
Now, we put the row insertion into table~\sqlil{g} into a \pgls{CTE}~\sqlil{g_new} by writing \sqlil{WITH g_new AS (INSERT INTO g (fkhid, x) VALUES (1, '123') RETURNING id, fkhid)}\sqlIdx{WITH}.
This \pgls{CTE} will insert a row into table~\sqlil{g} that references the row of table~\sqlil{h} that has primary key~1.
It will also return the primary key of the newly generated row in table~\sqlil{g} as~\sqlil{gid} and the foreign key~\sqlil{fkhid}~(with value~1).
Then we use this \pgls{CTE} when updating the row with primary key~\sqlil{hid = fkhid} in table~\sqlil{h}.
We do \sqlil{UPDATE h SET fkgid = g_new.gid FROM g_new WHERE h.gid = g_new.fkhid;}.
...
Relating the existing rows in table~\sqlil{g} to additional rows in table~\sqlil{h} is then much easier.
This can be done with single \sqlilIdx{UPDATE}~statements applied to table~\sqlil{h}.
...
First, we check whether it is possible to make a second row in table~\sqlil{g} relate to a row in table~\sqlil{h} that is already related to another row in table~\sqlil{g}.
