...
A similar structure can be implemented now.
...
Since each row in table~\sqlil{n} must be related to exactly one row in table~\sqlil{m}, we also add a column~\sqlil{fkmid} to this table.
...
This change is needed because every entity of type~N must be related to an entity of type~M, whereas back in \cref{sec:rm:gh}, each entity of type~H could be related to one or zero entities of type~G.
Apart from this difference, we now add the constraint \sqlil{m_fknid_mid_fk} to table~\sqlil{m} which works exactly as \sqlil{g_fkhid_gid_fk} in \cref{lst:GH_tables}.
...
This is a much worse problem, because we need to know the primary key~\sqlil{nid} of a row in table~\sqlil{n} in order to create a new row in table~\sqlil{m}.
And we need to know the primary key~\sqlil{mid} of a row in table~\sqlil{m} to create a new row in table~\sqlil{n}.
...
We need to, somehow, be able to get the value of the primary key that would be used when we create a new row in Tabelle~\sqlil{m} \emph{before} creating this row.
If we know the primary key value, then we can use its value as \sqlil{fkmid} when inserting a row into table~\sqlil{n}.
This way, get the primary key of that new row in Table~\sqlil{n}.
...
It is \sqlil{GENERATED BY DEFAULT}, which means that its values or generated automatically \emph{unless specified}~\cite{PGDG:PD:GC}.
...
Instead of using an implicit sequence without name, we also create a sequence by ourselves.
This can be done with the command~\sqlil{CREATE SEQUENCE}\sqlIdx{CREATE!SEQUENCE}\sqlIdx{SEQUENCE!CREATE}~\cite{PGDG:PD:CS}.
We create a sequence~\sqlil{sqmid} to generate the primary key values for the table~\sqlil{m} in \cref{lst:MN_tables} by writing~\sqlil{CREATE SEQUENCE sqmid AS INT;}\sqlIdx{CREATE!SEQUENCE}\sqlIdx{SEQUENCE!CREATE}.
The values in such sequences will always be strictly monotonously increasing.
...
This definition says that the value of the primary key~\sqlil{mid} \emph{can} be provided when creating rows in table~\sqlil{m}.
...
The difference is that we now can also generate valid values for~\sqlil{mid} without creating rows in table~\sqlil{m}.
...
First, we generate the primary key value for the row that we want to insert into table~\sqlil{m} as \pgls{CTE} that just invokes \sqlil{NEXTVAL('sqmid')} and remembers the result, i.e., \sqlil{m_id AS (SELECT NEXTVAL('sqmid') AS new_mid)}.
...
\end{sloppypar}%
%
After this, we want to append another row to table~\sqlil{n} and link it to the now existing row in table~\sqlil{m}.
This can be done more easily via~\sqlil{INSERT INTO n (y, fkmid) VALUES ('CD', 1);}.
