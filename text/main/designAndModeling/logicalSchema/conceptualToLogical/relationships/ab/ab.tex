...
Since every entity of type~A can only be linked to at most one entity of type~B, each value of the primary key of table~\sqlil{a} must only appear in column~\sqlil{fkaid} of table \sqlil{relate_a_and_b} \emph{at most once}.
The same holds for the entities of type~B vice versa, which also means that each value~(of \sqlil{bid}) can occur at most once in column~\sqlil{fkbid} \emph{at most once}.
In other words:
...
Now we want to query some information about an entity~A, using~\sqlil{SELECT}, but also need information about the potentially related B~entity.
...
The \sqlil{UNIQUE} constraint on the column~\sqlil{fkaid} of~\sqlil{relate_a_and_b} prevents this.
Vice versa, the \sqlil{UNIQUE} constraint on column~\sqlil{fkbid} in~\sqlil{relate_a_and_b} prevents that any row in table~\sqlil{b} is related to more than one row in table~\sqlil{a}.
...
We delete the tables we just created in \cref{lst:AB_cleanup} to explore the two-table solution in \cref{lst:AB_2_tables}.
The new column~\sqlil{fkbid} added to the table~\sqlil{a}.
It can be allowed to be~\sqlil{NULL}, because not all entities of type~A need to be related to an entity of type~B.
It must be marked as \sqlilIdx{UNIQUE}, though, because no entity of type~B can be related to more than one entity of type~A.
This constraint only applies to \sqlil{fkbid}~values that are \emph{not}~\sqlil{NULL}.

It also needs a \sqlil{REFERENCES} constraint.
In \cref{lst:AB_2_tables}, we add this constraint later via \sqlil{ALTER TABLE}\sqlIdx{ALTER!TABLE}\sqlIdx{TABLE!ALTER}, very much like \pgmodeler\ does it~(see back in \cref{lst:logical:teachingManagment:student_database_2:05_public_mobile_mobile_student_id_fk_constraint_5087}).
