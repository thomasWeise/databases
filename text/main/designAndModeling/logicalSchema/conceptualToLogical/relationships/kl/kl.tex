...
%
\gitExec{cdtrmTableK}{\databasesCodeRepo}{conceptualToRelational}{../_scripts_/db_table_to_latex_table.sh relationships k kid;x}{cdtrmTableK}%
...
For each such ID-type, we may have zero, one, or many personal~ID records stored in our \db.
...
The third table could never have just a few rows, it would always have exactly as many rows as Table~\sqlil{l}.
...
It does not need to be unique, because each row in table~\sqlil{k} can be related to arbitrarily many rows in table~\sqlil{l}.
...
It is impossible to add a row to table~\sqlil{l} without providing an existing value of the primary key attribute~\sqlil{kid} of table~\sqlil{k} for the foreign key column~\sqlil{fkkid}.
...
It cannot reference more than one row, because the atttribute~\sqlil{fkkid} can only take on a single value.
