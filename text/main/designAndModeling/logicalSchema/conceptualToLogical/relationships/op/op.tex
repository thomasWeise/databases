...
Now, since each row in table~\sqlil{o} can be related to multiple rows in table~\sqlil{p} and vice versa, no two-table solution is possible.
...
In \cref{lst:OP_tables}, we add the table \sqlil{relate_o_and_p} which has two columns,~\sqlil{fkoid} and~\sqlil{fkpid}.
These are foreign keys pointing to the primary keys~\sqlil{oid} and~\sqlil{pid} of tables~\sqlil{o} and~\sqlil{p}, respectively.
This is ensured with corresponding \sqlil{REFERENCES} constraints.

Both columns are marked as~\sqlil{NOT NULL}, because we only store valid O\nobreakdashes-P~pairs.
...
In the previous three-table solutions, we always had at least one \sqlil{UNIQUE} foreign key column.
That made sure that each pair of foreign key values could occur at most once.
Since we cannot have a single \sqlil{UNIQUE} column this time, it would now be possible that that the same \sqlil{(fkoid, fkpid)} values could occur multiple times.
Obviously, we cannot have that.
We must ensure that a pair~\sqlil{(fkoid, fkpid)} can appear at most once in the table.
Two specific rows in tables~\sqlil{o} and~\sqlil{p} can, of course, be related only once to each other.
...
Primary keys must be unique by default, so this constraint also covers the uniqueness.

In \cref{lst:OP_insert_and_select}, we now insert data into the two tables.
Since both relationship ends are optional, we can enter records in the two data tables arbitrary order.
We first enter some data into the tables~\sqlil{o} and~\sqlil{p}.
...
This would violate that rows must be unique, a basic requirement of the relational data model.
If that was possible, these rows would also appear twice in the result of the \sqlil{INNER JOIN}s when merging the data.
