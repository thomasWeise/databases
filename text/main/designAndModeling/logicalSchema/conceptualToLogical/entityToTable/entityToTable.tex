...
For example, if the \emph{Student} entity type in the conceptual model has the multi-valued attribute \emph{Mobile Phone}, this would mean that each entity of type \emph{Student} can have multiple values of \emph{Mobile Phone} associated with it.
...
Thus, multi-valued attributes need to become tables by themselves.
...
]{\includegraphics[width=0.45\linewidth]{\conceptualSchemaEntitiesAndAttributesDir/yedErdEntitiesA21erd}}%
%
\floatSep%
%
\subfloat[][%
...
Besides some comments, we only find the \sqlil{CREATE DATABASE student_database;}\sqlIdx{CREATE!DATABASE} statement.
...
There is not really anything there that goes beyond what we discussed in our initial example back in \cref{sec:factoryCreatingTableAndInsertingData}.
We see the \sqlil{CREATE TABLE}\sqlIdx{CREATE!TABLE} command together with statements for creating the constraints\sqlIdx{CONSTRAINT}.%
\end{sloppypar}%
%
...
%
\subfloat[][%
We want to design a model where composite and multi-valued attributes are represented. %
We therefore reprint the \pgls{ERD} from \cref{fig:erdStudent2} here.%
\label{fig:erdStudent22}%
]{\includegraphics[width=0.48\linewidth]{\conceptualSchemaEntitiesAndAttributesDir/erdStudent2}}%
%
\floatSep%
%
\subfloat[][%
...
%
\subfloat[][%
Multi-valued attributes should go into their own table. %
...
Multi-valued attributes, i.e., attributes that can take on several values for each entity, instead need to go into their own, separate tables.
Back when designing conceptual models, we had one variant of the \emph{Student} entity type design that had both a composite and a multi-valued attribute -- see \cref{fig:erdStudent2}.
...
We already used surrogate keys in our factory example back in \cref{sec:factoryCreatingTableAndInsertingData}.
...
Instead of directly including it when the table is created, the table is later changed~(\sqlilIdx{ALTER TABLE}).
