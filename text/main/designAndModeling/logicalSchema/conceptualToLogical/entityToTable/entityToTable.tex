...
Assume that \emph{Name} is not a simple attribute but a composite attribute.
Let's say that it consists of the two components \emph{Full Name} and \emph{Salutation}.
Then we will have two columns, one called~\sqlil{full_name} and one called~\sqlil{saluation}.
Both would have reasonable datatypes and attached sanity constraints.
In this case, the composite attribute~\emph{Name} in the conceptual model does not itself have a column in the table.
Instead, its components have columns.
Of course, if the components themselves are composite attributes, the process is repeated recursively.
Then, the components are broken down until we arrive at simple attributes, for which we then have columns.

Multivalued attributes become separate tables.
Each of their rows references the one row of the table of the entity type~(by storing its primary key via a foreign key).
...
In the relational data model, all datatypes are atomic, i.e., we cannot have a column that is of type \inQuotes{list of something}.
...
OK, so our goal here would be to either transform the conceptual model of a \db\ to a logical mode \emph{or}, in smaller-scale projects, to directly design the logical model.
...
This syntax can then be translated to \sql, which we can send to the \postgresql\ \pgls{server}, e.g., via the \psql\ \pgls{client}.
...
We type in \bashil{pgmodler} and hit~\keys{\return}. %
...
\listingSQLandOutput{logical:teachingManagment:student_database_1:01_student_database_database_2001}{%
...
\listingSQLandOutput{logical:teachingManagment:student_database_1:03_public_student_table_5071}{%
...
\listingSQLandOutput{logical:teachingManagment:student_database_1:insert}{%
...
\listingSQLandOutput{logical:teachingManagement:student_database_1:cleanup}{%
...
Under \ubuntu\ \linux, we can start this program by opening a \pgls{terminal} by hitting \ubuntuTerminal, typing in \bashil{pgmodler}, and hitting~\keys{\return}, as shown in \cref{fig:makeStudentTable01startPgmodeler}.
Under \microsoftWindows, you would instead proceed as shown in \cref{fig:installingPgModelerWindows22pgmodelerLogo}.
...
These two characters are the first two digits of the year of birth.
We do not permit \pglspl{dateOfBirth} before the year 19\textcolor{gray}{00} or after 20\textcolor{gray}{99}.
After that, we require nine digits to follow via~\textil{\\d\{9\}}.
We could do this a bit better to enforce that next two digits would be valid months~\intRange{\textil{01}}{\textil{12}} and that the two digits following that be valid days-of-month, i.e., \intRange{\textil{01}}{\textil{31}}.
In a \emph{real} application, we would totally do that {\dots} but here I leave this as exercise to the interested reader.
Anyway, this means we now have $6+2+9=17$~digits.
This leaves only the final checksum character, which can be any digit from 0~to~9 or~X.
...
One condition is that \sqlil{date_of_birth > '1900-01-01'}, i.e., we demand that students may not be born before the year~1900.
Another condition is that \sqlil{date_of_birth < '2100-01-01'}, which prevents students born in the 22nd century.
We combine both conditions with~\sqlilIdx{AND}.
...
It may be a cool idea to define a \sqlil{CHECK} constraint that validates whether the \pgls{dateOfBirth} given as \sqlil{date_of_birth} fits to the value provided in \sqlil{national_id}.
After all, the \pgls{dateOfBirth} is directly present in the national Chinese~ID number, too.
\sqlil{SUBSTR(national_id, 7, 8)}\sqlilIdx{SUBSTR} should give us the eight digits of the birth date from the national~ID~\cite{PGDG:PD:SFAO}.
\sqlil{TO_CHAR(date_of_bith, \'YYYYMMDD\')}\sqlIdx{TO\_CHAR} would probably give us the \pgls{dateOfBirth} in the same format~\cite{PGDG:PD:DTFF}.
Hence, we could add a \sqlil{CHECK}-constraint comparing the two and failing if they are different.
This would be a pretty strong guard against typos.
...
We see the \sqlil{CREATE TABLE}\sqlIdx{CREATE!TABLE}\sqlIdx{TABLE!CREATE} command together with statements for creating the constraints\sqlIdx{CONSTRAINT}. %
\end{sloppypar}%
%
The only peculiarity is the \sqlil{ALTER TABLE}\sqlIdx{ALTER!TABLE}\sqlIdx{TABLE!ALTER}, which can change a table after its creation~\cite{PGDG:PD:AT}.
Here, it assigns the owner of the table.
...
%
\subfloat[][%
We want to add the columns~\sqlil{full_name} and~\sqlil{saltulation} to represent the flattened composite attribute~\sqlil{name}.%
...
\listingSQLandOutput{logical:teachingManagment:student_database_2:01_student_database_database_2001}{%
This auto-generated \sql\ script creates the \db~\sqlil{student_database}.%
}{}%
%
...
\listingSQLandOutput{logical:teachingManagment:student_database_2:03_public_student_table_5071}{%
...
\listingSQLandOutput{logical:teachingManagment:student_database_2:04_public_mobile_table_5081}{%
...
\listingSQLandOutput{logical:teachingManagment:student_database_2:05_public_mobile_mobile_student_id_fk_constraint_5087}{%
...
\listingSQLandOutput{logical:teachingManagment:student_database_2:insert_and_select}{%
...
\listingSQLandOutput{logical:teachingManagment:student_database_2:cleanup}{%
...
We want to add the columns~\sqlil{full_name} and~\sqlil{saltulation} to represent the flattened composite attribute~\sqlil{name} in \cref{fig:newStudentDb02addColumns}.
...
Instead of directly including it when the table is created, the table is later changed~(\sqlil{ALTER TABLE}\sqlIdx{ALTER!TABLE}\sqlIdx{TABLE!ALTER}).
...
Wow, that was a lot.
We learned how we can translate an entity type from the conceptual model to a logical schema based on the relational data model.
Each entity type becomes one relation, i.e., one table.
Its attributes become the columns of the relation.
This, however, only holds for simple, single-valued attributes.
If the attribute is composite, we break it down to its atomic components and these become columns.
Multi-values attributes need to be separated from the entity type.
In relational \pglspl{db}, all datatypes are single-valued.
So multi-valued attributes become their own tables.
They are then linked over foreign key constraints back to the main table of the entity type.
Multiple rows in such a table can relate to one row of the main table, thus realizing the multi-valued-ness.

Luckily, we learned the new tool \pgmodeler.
This tool allows us to draw logical models for the \postgresql\ \dbms\ in a way quite similar to what we are used with \yEd.
The models we generated by \yEd\ were technology-independent conceptual models.
The logical models created with \pgmodeler\ are bound to \sql\ and the \postgresql\ \dbms.
We can export them as \pgls{SVG} graphics, exactly as we did with models we created in \yEd.
However, we can also export them to \sql\ directly.
This is actually pretty awesome.
We can now use a comfortable \pgls{GUI} to help us to build the \db\ schema.
And then we can directly feed it into \postgresql.%
\FloatBarrier%
\endhsection%
%
