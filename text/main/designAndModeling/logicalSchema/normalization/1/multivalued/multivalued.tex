...
\end{figure}%
%
\gitExecSQLraw{normalization:1nf:anomaly_multivalued:generated_sql:01_anomalies_database_2001}{}{normalization/1nf/anomaly_multivalued/generated_sql}{01_anomalies_database_2001.sql}{}{}{}%
%
\gitExecSQLraw{normalization:1nf:anomaly_multivalued:generated_sql:03_public_address_table_5071}{}{normalization/1nf/anomaly_multivalued/generated_sql}{03_public_address_table_5071.sql}{anomalies}{}{}%
%
...
%
\gitExecSQLraw{normalization:1nf:anomaly_multivalued:generated_sql:05_public_student_student_address_1_fk_constraint_5085.sql}{}{normalization/1nf/anomaly_multivalued/generated_sql}{05_public_student_student_address_1_fk_constraint_5085.sql}{anomalies}{}{}%
%
\gitExecSQLraw{normalization:1nf:anomaly_multivalued:generated_sql:06_public_student_student_address_2_fk_constraint_5086.sql}{}{normalization/1nf/anomaly_multivalued/generated_sql}{06_public_student_student_address_2_fk_constraint_5086.sql}{anomalies}{}{}%
%
...
%
\gitExecSQLraw{normalization:1nf:anomaly_multivalued:cleanup.sql}{}{normalization/1nf/anomaly_multivalued}{cleanup.sql}{}{}{}%
\afterpage{\clearpage}%
%
...
As you can see, we indeed have two foreign keys~(\sqlil{address_1} and \sqlil{address_2}) now pointing to table~\sqlil{address}.
We omitted printing the scripts for generating the foreign key \sqlilIdx{REFERENCES} constraints for the sake of brevity.
...
This violation of the \pgls{1NF} can cause various problems.
...
Column~\sqlil{address_2} must of course be permitted to be \sqlil{NULL}.
Therefore, a student has either one or two addresses.
Let's that we extended our model to three addresses.
Then it could happen that we get a row where the second address column is \sqlil{NULL} while the third address column is not, or the other way around.
...
Mrs.~Bibbi lives only in Chinatown, New York, USA.
...
In \cref{lst:1nf:anomaly_multivalued:select}, we therefore begin by figuring out how to get the data into the shape of a relation that has a one student name and one corresponding address foreign key value.
...
It then appends the result of a second query that does almost the same, but uses \sqlil{address_2} as \sqlil{adr}.
...
All that needs to be done is to write the subquery in parentheses~\sqlil{(...)}\sqlIdx{(\idxdots)}!
...
\sqlil{SELECT DISTINCT} deletes all duplicate rows resulting from a query.
In other words, if two or more rows have all the same values, only one of them is preserved and the remaining rows are dropped.
...
Well, names may be ambiguous.
We therefore choose to change our subqueries to also pass along the student~IDs~(renamed to~\sqlil{sid}).
...
If we want to do anything useful with multivalued attributes represented as repeating groups {\dots} then we have to rewire them be in \pgls{1NF}.
This means that we need to represent them as separate relations, that is, in the form that we already learned back in \dref{sec:mappingEntitiesToTables}.
...
\end{figure}%
%
\gitExecSQLraw{normalization:1nf:fixed_multivalued:generated_sql:01_fixed_database_2001.sql}{}{normalization/1nf/fixed_multivalued/generated_sql}{01_fixed_database_2001.sql}{}{}{}%
%
\gitExecSQLraw{normalization:1nf:fixed_multivalued:generated_sql:03_public_address_table_5071}{}{normalization/1nf/fixed_multivalued/generated_sql}{03_public_address_table_5071.sql}{fixed}{}{}%
%
...
The corresponding \sqlilIdx{REFERENCES} constraints have been omitted here for the sake of brevity.}%
%
\gitExecSQLraw{normalization:1nf:fixed_multivalued:generated_sql:06_public_student_address_student_fk_constraint_5087.sql}{}{normalization/1nf/fixed_multivalued/generated_sql}{06_public_student_address_student_fk_constraint_5087.sql}{fixed}{}{}%
%
\gitExecSQLraw{normalization:1nf:fixed_multivalued:generated_sql:07_public_student_address_address_fk_constraint_5088.sql}{}{normalization/1nf/fixed_multivalued/generated_sql}{07_public_student_address_address_fk_constraint_5088.sql}{fixed}{}{}%
%
...
%
\gitExecSQLraw{normalization:1nf:fixed_multivalued:cleanup.sql}{}{normalization/1nf/fixed_multivalued}{cleanup.sql}{}{}{}
...
But you do know how we could do this fully beautiful, so I hope that's OK.
...
We omit the script for creating the \db.
We also do not show the script for creating the table~\sqlil{address}, because it again was not changed.
In \cref{lst:1nf:fixed_multivalued:04_public_student_table_5079}, we create the table \sqlil{student}.
It has been modified based on our new model and now really only has the two columns \sqlil{id} and \sqlil{name}.
The table \sqlil{student_address}, created via \cref{lst:1nf:fixed_multivalued:05_public_student_address_table_5083}, ties student records and address records together.
By using the composite primary key, we enforce that a student can be connected to each address at most once, and vice versa.
This is because \sqlil{PRIMARY KEY}-constraints imply \sqlil{UNIQUE}.
We omitted the \sqlil{REFERENCES}-constraints here for brevity.

Let us now play with this newly normalized model.
We therefore generate the same data as before.
...
Notice that this step is now needed.
When the table was not in the \pgls{1NF}, we could directly link to addresses from the student records.
Now we need to go the extra step and insert records into one additional table.
This is the const of the \pgls{1NF}.
