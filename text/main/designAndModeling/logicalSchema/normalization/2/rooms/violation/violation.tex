...
\end{figure}%
%
\gitExecSQLraw{normalization:2nf:rooms:violation:generated_sql:01_violation_database_2001.sql}{}{normalization/2nf/rooms/violation/generated_sql}{01_violation_database_2001.sql}{}{}{}%
%
...
\listingSQLandOutput{2nf:rooms:violation:update_a}{%
...
\listingSQLandOutput{2nf:rooms:violation:update_b}{%
...
%
\gitExecSQLraw{normalization:2nf:rooms:violation:cleanup.sql}{}{normalization/2nf/rooms/violation}{cleanup.sql}{}{}{}%
%
...
Assume that a \db\ designer decided for a simpler implementation and chose to just put all the room and building data into a single table.
...
The \sqlil{building_number} and \sqlil{room_number} are both of type \sqlilIdx{VARCHAR} with a maximum length of~4.
They do not necessarily need to be numbers but could also be something like~\sqlil{'105b'}.
...
It creates the columns for the room and building names as well as the address as \sqlilIdx{VARCHAR} with a maximum length of~100.
The capacity for students of a room is an \sqlilIdx{INTEGER} number.
\sqlil{building_number} and \sqlil{room_number} together form the primary key.
...
We store information about two teaching rooms in the Language Teaching Building in South Campus~1, as well as some offices in Building~53 of South Campus~2, and some other rooms.
...
Besides the redundancy, this structure has another problem.
It becomes apparent when we try to change the data.
...
In this pattern, discussed in \dref{sec:rm:mn}, it would be required to delete the building data if all rooms in the building were removed.
...
\Cref{lst:2nf:rooms:violation:update_a} illustrates another issue that is encouraged~(but not caused) by a violation of the \pgls{2NF}: inconsistency.
Assume that we wanted to get a list of all rooms available in South Campus~2.
We could fire out the first query given in \cref{lst:2nf:rooms:violation:update_a}.
We could simply \sqlilIdx{SELECT} the building number and room number where \sqlil{address = 'South Campus 2'}.
...
Violating the \pgls{2NF} forces us to write the same information more often.
Hence it increases the chance of making an error.
...
Or we just fix the incorrectly entered data with an \sqlilIdx{UPDATE} command.
We would set the building address to \sqlil{'South Campus 2'} if it currently is \sqlil{'South Campus II'}.
...
This can be done with the query shown in \cref{lst:2nf:rooms:violation:update_b}.
...
We again use the \sqlilIdx{RETURNING} statement~\cite{PGDG:PD:RDFMR} to print out all the rows that were modified with this command.
As you can see, this changes three rows.
Notice that we wanted to change only \emph{one} single piece of data.
But we did~(or better:~needed to)~change \emph{three} rows.%
%
...
\end{definition}%
%
For example, it is simply not possible to insert information about a room without inserting information about a building as well~(and vice versa).
Of course, in our special case here, this would not make any sense anyway.
But there can be many other cases where the violation of the \pgls{2NF} causes this as a problem.

In~\cite{K1983ASGTFNFIRDT}, for example, another interesting case is presented from the field of warehousing.
The names and addresses of warehouses are stored together with the names and quantities of the products in them \emph{in one table}.
This design of that single table violates the \pgls{2NF}.
The data was stored in exactly these four columns~(name of warehouse, address of warehouse, name of product, quantity of product).
Thus, it is not possible to create a record for a warehouse if no product is stored in it as well.%
%
\FloatBarrier%
\endhsection%
%
