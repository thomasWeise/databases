...
\end{figure}%
%
\gitExec{\databasesCodeRepo}{.}{_scripts_/postgres.sh normalization/2nf/rooms/violation/generated_sql 01_violation_database_2001.sql}%
%
\gitSQL{\databasesCodeRepo}{normalization/2nf/rooms/violation/generated_sql/03_public_building_room_table_5071.sql}{2nf:violation:03_public_building_room_table_5071}{The generated \sql\ code for creating the table \sqlil{building_room} that violates the \pgls{2NF} based on \cref{fig:roomsViolation2nf:model}.}%
\gitExec{\databasesCodeRepo}{.}{_scripts_/postgres.sh normalization/2nf/rooms/violation/generated_sql 03_public_building_room_table_5071.sql violation}%
%
\gitSQL{\databasesCodeRepo}{normalization/2nf/rooms/violation/insert.sql}{normalization:2nf:rooms:violation:insert}{%
Inserting some data into the table~\sqlil{building_room} in violation of the \pgls{2NF}.}%
\gitExec{\databasesCodeRepo}{.}{_scripts_/postgres.sh normalization/2nf/rooms/violation insert.sql violation}%
%
\gitSQLAndOutput{\databasesCodeRepo}{normalization/2nf/rooms/violation}{delete.sql}{violation}{}{}{postgres.sh}{normalization:2nf:rooms:violation:delete}{%
...
%
\gitSQLAndOutput{\databasesCodeRepo}{normalization/2nf/rooms/violation}{update.sql}{violation}{}{}{postgres.sh}{normalization:2nf:rooms:violation:update}{%
...
%
\gitSQLAndOutput{\databasesCodeRepo}{normalization/2nf/rooms/violation}{update2.sql}{violation}{}{}{postgres.sh}{normalization:2nf:rooms:violation:update2}{%
...
%
\gitExec{\databasesCodeRepo}{.}{_scripts_/postgres.sh normalization/2nf/rooms/violation cleanup.sql}%
%
...
In \cref{lst:normalization:2nf:rooms:violation:insert}, we fill the table with some data.
...
With the first query in \cref{lst:normalization:2nf:rooms:violation:delete}, we want to extract a list of all buildings from our \db.
...
We now run the same query giving us the list of buildings again~(as the third query in \cref{lst:normalization:2nf:rooms:violation:delete}).
...
\Cref{lst:normalization:2nf:rooms:violation:update} illustrates another issue that is encouraged~(but not caused) by a violation of the \pgls{2NF}: inconsistency.
Assume that we wanted to get a list of all rooms available in South Campus~2.
We could fire out the first query given in \cref{lst:normalization:2nf:rooms:violation:update}, namely we could simply \sqlilIdx{SELECT} the building number and room number where \sqlil{address = 'South Campus 2'}.
...
Scrolling back to our insertion script in \cref{lst:normalization:2nf:rooms:violation:insert}, we noticed that we sometimes wrote~\sqlil{'South Campus II'} instead of~\sqlil{'South Campus 2'}.
...
This can be done with the query shown in \cref{lst:normalization:2nf:rooms:violation:update2}.
