...
\end{figure}%
%
\gitExecSQLraw{normalization:2nf:rooms:fixed:generated_sql:01_fixed_database_2001.sql}{}{normalization/2nf/rooms/fixed/generated_sql}{01_fixed_database_2001.sql}{}{}{}%
%
...
\listingSQLandOutput{2nf:rooms:fixed:update_a}{%
We want to see all rooms in South Campus~2. %
Compared to \cref{lst:2nf:rooms:violation:update_a}, the query is more complex as we now need an \sqlilIdx{INNER JOIN}. %
...
In \cref{exec:2nf:rooms:violation:update_a}, two rows were affected.%
}{}%
%
\gitLoadAndExecSQL{2nf:rooms:fixed:update_b}{}{normalization/2nf/rooms/fixed}{update_b.sql}{fixed}{}{}%
\listingSQLandOutput{2nf:rooms:fixed:update_b}{%
We want to change the name of Building~36 to \inQuotes{Computer Science Building.} %
Different from the situation in \cref{exec:2nf:rooms:violation:update_b}, only a single row needs to be changed.%
}{}%
%
\gitExecSQLraw{normalization:2nf:rooms:fixed:cleanup.sql}{}{normalization/2nf/rooms/fixed}{cleanup.sql}{}{}{}%
%
The violation of the \pgls{2NF} can -- either directly or indirectly -- cause several problems or anomalies.
So let us normalize the \db\ from the previous section into the \pgls{2NF}.

In our original design, we had a single table \sqlil{building_room}.
This table had a composite primary key consisting of the columns \sqlil{building_number} and \sqlil{room_number}.
However, the columns \sqlil{building_name} and \sqlil{address} only depend on \sqlil{building_number}.
This led to several anomalies, to redundancy, and increased the probability to make errors.
...
We can use \sqlil{building_number}, because, in our university, the rule is that there can never be two buildings with the same number.
...
This table still has the composite primary key consisting of the columns~\sqlil{building_number} and~\sqlil{room_number}
There is no reason to change this.
Matter of fact, keeping this composite key instead of using a surrogate key directly enforces that rooms can only be stored \emph{after} the corresponding building record has been created.

There also still are the two fields~\sqlil{room_name} and~\sqlil{capacity} in this table.
Clearly, both non-key fields are functionally dependent on the primary key.
They depend on the \emph{complete} primary key, to be exact.
As a bonus additionally to satisfying the \pgls{2NF}, this logical design, illustrated in~\cref{fig:roomsFixed2nf}, represents our original conceptual model given in \cref{fig:roomsErdRoom2} much better.

We now implement this design using \sql.
\Cref{lst:2nf:rooms:fixed:04_public_building_table_5080} creates the table \sqlil{building}.
It has the columns \sqlil{building_number}, \sqlil{building_name}, and \sqlil{address}.
\sqlil{building_number} is the primary key.
We could have added a \sqlil{UNIQUE} constraint for \sqlil{building_name}, thus enforcing that no two buildings can have the same name {\dots} but we leave this as an exercise for the interesting reader.

\Cref{lst:2nf:rooms:fixed:03_public_room_table_5071} creates the table~\sqlil{room}.
Its first two columns, \sqlil{building_number} and \sqlil{room_number}, form the composite primary key.
The other two columns~(\sqlil{room_name} and \sqlil{capacity}) functionally depend on this key in its entirety.
\Cref{lst:2nf:rooms:fixed:05_public_room_building_fk_constraint_5099} establishes the foreign key constraint linking each row of table~\sqlil{room} to one row of table~\sqlil{building}.
This constraint turns \sqlil{building_number} into a foreign key.

We can now insert the exactly as same data as in the last section into this \db.
\Cref{lst:2nf:rooms:fixed:insert} therefore now needs two~\sqlilIdx{INSERT INTO} statements.
...
The data about each building is stored exactly once.
Before, we needed to store it for each room.
...
Back then, we first got a list of all buildings and found that there were four of them.
We then deleted the two rooms 904a and~904b.
This led to Building~53 to disappear as well, as its existence only emerged from the two room records.

So we again start by geting a list of all buildings.
...
Back then, in the table~\sqlil{building_room}, each building number and name could appear several times, as they were stored for each room.
However, in our new table~\sqlil{building}, they can only appear once.
Hence, the omission of duplicate rows, performed by \sqlilIdx{DISTINCT}, is now no longer necessary.
...
In \cref{lst:2nf:rooms:fixed:update_a}, we reproduce the example from \cref{lst:2nf:rooms:violation:update_a}:
...
The room information is stored in table~\sqlil{room}.
...
This problem can be fixed exactly in the same ways as back in \cref{lst:2nf:rooms:violation:update_a}.
...
We also return the building number of the buildings that are affected by this update.
As the result, we see that a single row in the table~\sqlil{building} is changed.
Back in \cref{lst:2nf:rooms:violation:update_a}, two rows needed to be changed.
That was an example of the update anomaly.
And that anomaly is gone now.
After the update, the first query works as expected.

As a side note:
The fact that this problem still persists hints that we should probably have a table \sqlil{building_address} where we put the building addresses.
This would avoid this issue.

An even clearer example of the update anomaly that occurs when the \pgls{2NF} is violated was given in \cref{lst:2nf:rooms:violation:update_b}.
...
Now that we observe the \pgls{2NF}, doing the same thing in \cref{lst:2nf:rooms:fixed:update_b} only affects a single row in table~\sqlil{building}.
