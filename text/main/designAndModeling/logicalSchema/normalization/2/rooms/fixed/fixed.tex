...
%
\gitSQL{\databasesCodeRepo}{normalization/2nf/rooms/fixed/generated_sql/04_public_building_table_5080.sql}{2nf:fixed:04_public_building_table_5080}{The generated \sql\ code for creating the table~\sqlil{building}.}%
...
%
\gitSQL{\databasesCodeRepo}{normalization/2nf/rooms/fixed/generated_sql/03_public_room_table_5071.sql}{2nf:fixed:03_public_room_table_5071}{The generated \sql\ code for creating the table~\sqlil{room}.}%
...
%
\gitSQL{\databasesCodeRepo}{normalization/2nf/rooms/fixed/generated_sql/05_public_room_building_fk_constraint_5099.sql}{2nf:fixed:05_public_room_building_fk_constraint_5099}{The generated \sql\ code for creating the foreign key constraint linking the rows in table~\sqlil{room} to the rows in table~\sqlil{building}.}%
...
%
\gitSQL{\databasesCodeRepo}{normalization/2nf/rooms/fixed/insert.sql}{normalization:2nf:rooms:fixed:insert}{%
Inserting the very same data as in \cref{lst:normalization:2nf:rooms:violation:insert} into the tables~\sqlil{building} and \sqlil{room}.}%
\gitExec{\databasesCodeRepo}{.}{_scripts_/postgres.sh normalization/2nf/rooms/fixed insert.sql fixed}%
%
\gitSQLAndOutput{\databasesCodeRepo}{normalization/2nf/rooms/fixed}{delete.sql}{fixed}{}{}{postgres.sh}{normalization:2nf:rooms:fixed:delete}{%
...
This is different from the situation in \cref{exec:normalization:2nf:rooms:violation:delete}, where it disappeared.}%
%
\gitSQLAndOutput{\databasesCodeRepo}{normalization/2nf/rooms/fixed}{update.sql}{fixed}{}{}{postgres.sh}{normalization:2nf:rooms:fixed:update}{%
We want to see all rooms in South Campus~2. %
Compared to \cref{lst:normalization:2nf:rooms:violation:update}, the query is more complex as we now need an \sqlilIdx{INNER JOIN}. %
...
In \cref{exec:normalization:2nf:rooms:violation:update}, two rows were affected.%
}%
%
\gitSQLAndOutput{\databasesCodeRepo}{normalization/2nf/rooms/fixed}{update2.sql}{fixed}{}{}{postgres.sh}{normalization:2nf:rooms:fixed:update2}{%
We want to change the name of Building~36 to \inQuotes{Computer Science Building.} %
Different from the situation in \cref{exec:normalization:2nf:rooms:violation:update2}, only a single row needs to be changed.%
}%
%
...
\Cref{lst:normalization:2nf:rooms:fixed:insert} therefore needs two~\sqlilIdx{INSERT INTO} statements.
...
Notice that, compared to \cref{lst:normalization:2nf:rooms:violation:delete}, we do no longer need to write the \sqlilIdx{DISTINCT} keyword.
...
In \cref{lst:normalization:2nf:rooms:fixed:update}, we reproduce the example from \cref{lst:normalization:2nf:rooms:violation:update}:
...
A clearer example of the update anomaly that occurs when the \pgls{2NF} is violated was given in \cref{lst:normalization:2nf:rooms:violation:update2}.
...
Now that we observe the \pgls{2NF}, doing the same thing in \cref{lst:normalization:2nf:rooms:fixed:update2} only affects a single row in table~\sqlil{building}.
