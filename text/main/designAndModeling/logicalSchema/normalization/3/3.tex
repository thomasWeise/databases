...
%
The \glsFull{3NF} deals with the relationship between non-key attributes~\cite{C1971FNOTDBRM,C1971NDBSABT,K1983ASGTFNFIRDT,D2003AITDS,EN2015FODS}.
...
Let $A$, $B$, and~$C$ be three distinct attributes (or distinct sets of attributes) of the relation~$R$, i.e, $A\subseteq\relSchemab{R}$, $B\subseteq\relSchemab{R}$, and~$C\subseteq\relSchemab{R}$. %
...
\end{definition}%
%
Formally, the \pgls{3NF} can be stated as follows~\cite{SS2005EIDDDFDB:SDWSD2}:%
%
\begin{definition}[\glsreset{3NF}\Pgls{3NF}]%
\label{def:3nf}%
A relation~$R$ is in \pgls{3NF} if it is in~\pgls{2NF} and it is true for each attribute~$a\in\relSchemab{R}$ that is transitively dependent on a key~$X\subseteq\relSchemab{R}$ -- i.e., for which it holds that~$\funcDepb{X}{\funcDepb{Y}{a}}$ with~$Y\subseteq\relSchemab{R}$ -- that either~$Y$ contains a key, $a$~is part of the primary key, or~$a\in X$.%
\end{definition}%
%
