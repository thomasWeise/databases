...
\caption{A new version of the \emph{Student} \pgls{ERD} from \cref{fig:yedErdEntitiesA21erd}, this time with \emph{Name} as composite attribute.}%
\label{fig:erdStudent2}%
\end{figure}%
%
First, the \emph{Name} attribute should not be a single attribute.
...
This sounds a like an awful problem that would probably become the source of many interesting errors.
...
A composite attribute in the \pgls{ERD} is simply drawn as ellipse with the components connected to the attribute.
...
Multi-valued attributes can be modeled by ellipses with small ellipses inside.%
\FloatBarrier%
\endhsection%
%
