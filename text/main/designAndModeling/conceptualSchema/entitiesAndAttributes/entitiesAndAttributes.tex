...
\end{definition}%
%
Examples of entities are maybe the student Mr.~Bibbo, the module~\citetitle{programmingWithPython}~\cite{programmingWithPython}, the professor Mrs.~Bebba~教授, room~\#36~202, or the curriculum \emph{Computer Science and Technology}.
...
A student could be defined by their name, ID, student~ID, mobile phone number, home address, \pgls{dateOfBirth}, etc.
...
A \pgls{dateOfBirth}, on the other hand, is a special date whose year is from a certain range of reasonable years.
...
Each entity (type) should model exactly one (type of) object from reality (and not more than one)~\cite{SS2005EIDDDFDB:NQORD}.%
}%
%
...
The modules~\citetitle{programmingWithPython}~\cite{programmingWithPython} and \citetitle{databases}~\cite{databases} form an entity set.
...
Second, if you attended or do attend a course on programming~(such as~\cite{programmingWithPython}), then you will feel that this way of modeling things is a bit related to \glsreset{OOP}\pgls{OOP}.
Entity types could be thought of as \pythoniles{class} and entities could be their instances.
...
For this, a graphical method has been introduced:
...
\Citeauthor{SS2005EIDDDFDB:CDDRAAML}~\cite{SS2005EIDDDFDB:CDDRAAML} has two baseline variants and includes several slight variations of a \pgls{UML}-based approach~(which is not listed in~\cite{S2024D:CDMERDE}).
...
%
\subfloat[][%
We double-click into the new entity symbol in the document in order to edit its name.%
...
%
\subfloat[][%
We change the entity type name to \inQuotes{Student} and press \keys{\return}.%
...
%
\subfloat[][%
The name has changed. We now click on the \menu{Attribute} symbol in the palette.%
...
%
\caption{Drawing an \pgls{ERD} for the \emph{Student} entity type using \yEd.}%
...
%
\subfloat[][%
We then click into the entity type to connect the attribute \emph{Name} to the \emph{Student} entity type.%
...
%
\caption{Drawing an \pgls{ERD} for the \emph{Student} entity type using \yEd~(Continued).}%
...
%
\subfloat[][%
The \emph{Name} attribute is now connected to the \emph{Student} entity type.%
...
%
\subfloat[][%
We can also export the diagram in a format that we can use in other documents. %
...
So, we will not just look at \pglspl{ERD}, we will also \emph{draw} them.
...
We will use it for all of the conceptual-level \pglspl{ERD} in the rest of the book.
As an example on how to use \yEd, we will give some instructions on how to draw the most simplest \pgls{ERD} with only a single entity type inside in \cref{fig:yedErdEntitiesA:A}.
...
At least we do not just paint some \pglspl{ERD} and leave you entirely to your own devices when the time comes where you should draw some as well{\dots}

Before we really get into this, just a quick note:
In the context of \pglspl{ERD}, an \emph{entity type} are sometimes called a \emph{entity}.
...
From our requirements analysis, we know that students have names, they have an ID~(中国公民身份号码\cite{GB116431999CIN}, issued by the government), they have a student ID (issued by our university), they have a mobile phone number, they have an address, and they have a \pgls{dateOfBirth}.
...
We change the entity type name to \inQuotes{Student} and press \keys{\return} in \cref{fig:yedErdEntitiesA05changeNameToStudent}.
...
For example, the name of \inQuotes{Mr.~Bebbo} would actually be \inQuotes{Mr.~Fred Bebbo}, where \inQuotes{Bebbo} is the family name, \inQuotes{Fred} is the given name.
...
A multivalued attribute can be modeled as an ellips with a smaller ellipse inside in an \pgls{ERD}.
Optional attributes can be signified in the \pgls{ERD} by small empty bubbles at the end of the lines connecting them to the entity types.
...
Derived attributes are illustrated in \pglspl{ERD} by ellipses with dashed lines around their perimeter, as shown in \cref{fig:erdStudent4}.
