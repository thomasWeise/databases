...
We did not explicitly state this before, but model follows a simple principle:%
%
\bestPractice{immutableData}{%
If a situation changes, existing data should not be changed, but new data should be added that represents the change.}%
%
For example, a curriculum consists of modules.
Now we could assign each module to a teacher.
Once the teacher changes their workstation and another teacher takes over, we could just change the assignment.
This would have a big drawback:
...
Our system may also have some shortcomings.
...
But all in all, the system looks more or less reasonable.%
%
\FloatBarrier%
\endhsection%
%
