...
A modality of \inQuotes{optional participation} means that the minimum participation number is~0.
In this case, one entity occurrence does not require a corresponding entity occurrence in a particular relationship.
A modality of \inQuotes{mandatory participation} means that the minimum participation number is~1.
...
In the similar modelling language \pgls{UML}, cardinality is called multiplicity.

\begin{figure}%
\centering%
\includegraphics[scale=0.6]{\currentDir/erdCardinalities1}%
\caption{Three different ways to express possible modalities and cardinalities of relationship ends in \pglspl{ERD}.}%
...
Here, an integer range~\intRange{i}{j}, where $i$~is the minimum number of participating entities, $j$~is the maximum, and $j=*$~stands for unlimited, multiple, many, or~infinity.
...
Cardinalities like \intRange{7}{11} could potentially be nightmarish to implement and enforce later on.

In \libreofficeBase, \pglspl{ERD} can be drawn that are directly linked to the underlying \db.
They are in the 1:1/1:n notation and the lowercase~n represents \inQuotes{multiple}.
...
As said, we will stick with the Crow's Foot notation.
...
Now move your finger towards the~L and read the relationship end there, which says:~\inQuotes{{\dots}zero, one, or multiple~L.}~\cite{SE:DA:2016HDIRENCFTCTNL}.
Then place your finger on the~L and say \inQuotes{Each~L has\dots}.
Move the finger to the~K and read the relationship end touching the~K.
The symbol there means that~\inQuotes{{\dots}exactly one~K.}
...
\includegraphics[scale=0.6]{\figErdPersonIV}%
...
We introduce the weak entity position that we use to link faculty members to position types.
