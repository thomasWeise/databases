...
And again, if these entity types were unrelated, say \inQuotes{Student,} \inQuotes{Weather,} \inQuotes{Product,} then we were still better off storing them in a bunch of files.
...
\end{definition}%
%
An example of a relationship would be \emph{Mr.~Bibbo enrolls into the module \citetitle{programmingWithPython}.}
%
...
The professor entity would  have the role \emph{teaches}.%
%
...
It neither belongs to the student \emph{Mr.~Bebbo} nor does it belong to the module~\emph{\citetitle{databases}}.
...
Relationships~(or better, relationship types) in \pglspl{ERD} are drawn as diamonds that are connected to the involved entities~(or better, entity types).
\Cref{fig:erdStudentModule1} shows an \pgls{ERD} where the student entity type is linked to a module entity type by the relationship type \emph{enrolls into}.
...
Maybe in this year, Professor \citeauthor{databases} teaches the module \citetitle{databases}, maybe next year Professor~李 teaches it.
