...
This conceptual schema must then be mapped to a physical schema.
...
Now, \dbs\ are special technological objects.
However, they are objects with a comparatively long lifetime~\cite{SS2005EIDDDFDB:I}.
...
\caption{The waterfall model~\cite{PM2020SEAPA,GMTM2011DDLC}.}%
\label{fig:model:waterfall}%
\end{figure}%
%
Several different design methods have been proposed in the field of software engineering.
...
\caption{The \pgls{RAD} model~\cite{S2007OOSE,GMTM2011DDLC}.}%
\label{fig:model:rad}%
\end{figure}%
%
\glsreset{RAD}\Pgls{RAD}, as illustrated in \cref{fig:model:rad}, lets the user try the application before it is delivered~\cite{S2007OOSE,GMTM2011DDLC}.
...
A student management \db\ for a university, for example, may offer the students to log in and join courses or view their grades.
It may also allow the administrative staff to create new courses and even manage the module structure of curricula.
It may furthermore handle the room planning for courses.
...
They often are adaptations of different software lifecycle models and can offer different levels of detail to the project managers.
...
In the context of this course, our focus is on \pglspl{rdb}, where data is store in tables that can reference each other.
...
In our course, we chose \postgresql, simply because it is a free (open source) and very mature \sql\ \dbms.

In the next step, the conceptual design is converted into a logical design.
This can mean to map the entity types from the \pglspl{ERD} in the conceptual models to tables and the relationships between them~\cite{SS2005EIDDDFDB:I,SS2005EIDDDFDB:CDD}.
...
Some approaches only distinguish the conceptual and the physical schema~\cite{G2011EW2ITDS:ITRD,V1999C5DMS:L1CI}.
...
\begin{figure}%
\centering%
\includegraphics[width=0.8\linewidth]{\currentDir/schweppe}%
\caption{The lifecycle model by~\citeauthor{SS2005EIDDDFDB:I}~\cite{SS2005EIDDDFDB:I,SS2005EIDDDFDB:CDD}.}%
\label{fig:model:schweppe}%
\end{figure}%
%
As stated before, we now want to explore the design of \db\ by working our way along a \db\ lifecycle and development process.
However, the method discussed above and sketched in \cref{fig:model:gupta} may be a bit too comprehensive for such an example.
The most important core of such a process is condensed in the design model defined by \citeauthor{SS2005EIDDDFDB:I} in~\cite{SS2005EIDDDFDB:I,SS2005EIDDDFDB:CDD} and illustrated in \cref{fig:model:schweppe}.
It is more focused on the immediate \db\ design, omitting several of the framework activities.
While I very much like the comprehensive \db\ development lifecycle by~\citeauthor{GMTM2011DDLC}~\cite{GMTM2011DDLC}, at this stage, let us agree that project management, planning, and maybe systems security are a bit on the fringes of the topic of this book.
They can probably be elaborated better in other courses.
Let us work our way along the more concentrated process in \cref{fig:model:schweppe} as a second, more comprehensive example of \db\ technology.%
%
\FloatBarrier%
\endhsection%
%
