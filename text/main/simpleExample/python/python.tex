...
\listingPythonAndOutput{factory:connect_and_insert_1}{A \python\ program connecting to our factory \pgls{db} and then using \sqlilIdx{INSERT INTO} to insert one record.}{}%
%
We can do more.
...
\pythonilIdx{LiteralString} is a type annotation for string~\emph{\pglspl{literal}}.
...
Now the full \sqlilIdx{INSERT INTO} statement is rather long and does not fit on a single line in \cref{lst:factory:connect_and_insert_1}.
...
When programs access our \pglspl{db}, then we need to make sure that these programs cannot be used to attack our system.
...
Trying to manipulate the input data of programs that access \pglspl{db} is a very old and yet very common attack vector~\cite{S2008SI}.
...
Also, before, we used this strange type \pythonilIdx{LiteralString}.
What do we need that for?
We did not give a reasonable explanation.
The explanation is:~systems security.

\pythonilIdx{LiteralString} is basically a special string type, a subclass of \pythonil{str}, for strings \pglspl{literal}, i.e., strings that were written as text in \python.
If a value or variable is of type \pythonilIdx{LiteralString}, then it is still a string.
However, this string is explicitly declared to not be the result of any string operation, not be the result of concatenation, and no \pgls{fstring} either.
So we do not just not use \pglspl{fstring}, we even explicitly state that we do not use them intentionally.
Why would we do that?%
%
\begin{sloppypar}%
\Pglspl{SQLi}~\cite{ASO2024SIADPP,KDP2024AOSIAITCAIWA,CMCVGHRDCAAFL2023SIADINFD,S2008SI,databases} are attacks on \sql\ \pglspl{server} that are made possible if the \sql\ queries are constructed via string concatenation~(\pythonil{+}) or \pgls{strinterpolation}.
For example, assume that we would construct our query like this:~\pythonil{"INSERT INTO demand (customer, product, amount, ordered) VALUES (" + s + ")"}.
Then someone could set the string~\pythonil{s} to \pythonil{s = "1, 2, 3, '2023-02-02'); DROP TABLE demand;"}\sqlIdx{DROP!TABLE}.
The \textil{);} in \pythonil{s} would terminate the insertiong query.
The rest of the string would then be a new query: \sqlil{DROP TABLE...} would delete the entire table~\sqlil{demand}\footnote{We will learn that later, in \cref{sec:simpleExampleCleanup}.}.
...
\end{sloppypar}%
%
\pythonilIdx{LiteralString} is provided by \python\ for security reasons~\cite{PEP675}, with the goal to prevent such \pglspl{SQLi}.
The idea is that, in the future, static type checking tools could check whether a string is constructed or a \pgls{literal} and raises errors in security critical situations~\cite{PEP675,VDGE2022PPDAFP:ST}.
It is forbidden to use any form of string construction during the assembly of an \sql\ command in \psycopg.
It is enforced that each command be a string \pgls{literal}, i.e., of type~\pythonilIdx{LiteralString}.
Therefore, attacks exploiting string operations as vector are prevented~(as long as we follow these rules).

But then the question arises:
OK, no \pglspl{fstring} {\dots} then how do we get parameter values into the queries?
If all queries would need to be static, fixed, hard-coded string \pglspl{literal}, then we couldn't do many useful things, could we?
For this the \pythonil{"\%s"} exist as placeholders for query parameter.
The idea is that the system can apply proper escaping, meaning that all characters that could lead to problems are replaced with proper \pglspl{escapeSequence} and hence turned into harmless text.
...
Otherwise, you open the door to \pglspl{SQLi}. %
...
\listingPythonAndOutput{factory:connect_and_insert_2}{A \python\ program connecting to our factory \pgls{db} and then using \sqlilIdx{INSERT INTO} to insert multiple records.}{}%
After inserting one record via the method~\pythonilIdx{execute}, we now use the \pythonilIdx{executemany} method to insert multiple records in \cref{lst:factory:connect_and_insert_2}.
...
The output produced by \cref{lst:factory:connect_and_select} if we execute it a second time after running \cref{lst:factory:connect_and_insert_2}.%
}{,style=text_style}%
%
...
Repeating the query from \cref{lst:factory:select_from_view_sale_1} after inserting the records using our \python\ program from \cref{lst:factory:connect_and_insert_2}.%
}{}%
%
