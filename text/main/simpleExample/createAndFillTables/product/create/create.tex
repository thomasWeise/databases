...
Then we can send the \sql\ commands to our \postgresql\ \dbms\ and we will have a fresh and nice \sqlil{product}~table.
...
If we chose the manual method, then we first start a new \psql\ session in \cref{fig:createTable1psql}.
...
So what does the \sqlil{CREATE TABLE}\sqlIdx{CREATE!TABLE} command in \cref{fig:createTable3command} look like and why does it look like that?
...
We begin with the column \sqlil{name}.
...
\sql\ provides the datatype \sqlilIdx{VARCHAR}, which is used for variable-length strings, whose maximum length is specified in parentheses~\cite{PGDG:PD:CT}.
...
Now prices naturally are values with fractions, something like \$99.99 or 17.75元 or 3.79€, so at first glance, the \sql\ equivalent of \python's \pythonilIdx{float} seems a reasonable choice here.
However, it is actually not, because in our companion book \citetitle{programmingWithPython}~\cite{programmingWithPython}, we can find the following information:%
%
...
We therefore annotate the column \sqlil{price} with the \sqlilIdx{NOT NULL} constraint.

We also already explained that we will sell each unit of a product in a box.
