...
Therefore, we add the \sqlilIdx{NOT NULL} specification and write \sqlil{name VARCHAR(100)\ UNIQUE NOT NULL}\sqlIdx{VARCHAR}\sqlIdx{NOT NULL}\sqlIdx{UNIQUE}.
...
%
\begin{sloppypar}%
Having a reasonable limitation for the name, we now define the rule \sqlil{customer_name_ok} as \sqlil{CONSTRAINT customer_name_ok CHECK (name \~ '^\\w.*\\w\$')}\sqlIdx{{\textbackslash}w}\sqlIdx{\$}\sqlIdx{\^}\sqlIdx{.}\sqlIdx{\textasciitilde}.
...
\sqlil{(name \~ '^\w.*\w\$')}\sqlIdx{{\textbackslash}w}\sqlIdx{\$}\sqlIdx{\^}\sqlIdx{.}\sqlIdx{\textasciitilde} says that the field \sqlil{name} must match the \pgls{regex} \sqlil{'^\\w.*\\w\$'}.
...
We write \sqlil{CONSTRAINT customer_phone_ok CHECK (phone ~ '^\d\{10,11\}\$')}\sqlIdx{CONSTRAINT!CHECK}\sqlIdx{{\textbackslash}d}\sqlIdx{\$}\sqlIdx{\^}\sqlIdx{\textasciitilde}.
...
The \pgls{regex} \sqlil{'\^\\d\{10,11\}\$'}\sqlIdx{{\textbackslash}d}\sqlIdx{\$}\sqlIdx{\^} reads as follows:
...
Never use \sql\ keywords or reserved words as names, e.g., for columns or tables~\cite{B2025SBPASG}.%
}%
%
...
If we are about to insert a record into our \sqlil{demand} table where someone orders more than a million shoes, chances are that something went wrong.
So we want to define a constraint enforcing \sqlil{amount} to stay in~\intRange{1}{1\decSep000\decSep000}.
...
It allows us to specify dates in the ISO standard format~\inQuotes{\textil{YYYY-MM-DD}}~\cite{ISO860112019}, where \textil{YYYY} is the four-digits of the year, \textil{MM} stands for the number month in two digits, and \textil{DD} is the date specified with two digits as well.
...
Querying \sqlil{SELECT * FROM demand;}\sqlIdx{SELECT} will show us all the rows of this table.
