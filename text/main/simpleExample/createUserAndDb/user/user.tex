...
%
\subfloat[][%
We quit this session by typing in the commant~\textil{\\q} and hit~\keys{\return}.%
...
Anyway, we type in the command and hit~\keys{\return}.
...
We type it in \cref{fig:psqlNewUser5getUsers} and hit~\keys{\return}.
...
%
We can now close this \psql\ session by typing~\sqlil{\\q}\sqlIdx{{\textbackslash}q} and hitting~\keys{\return}.
...
In such a session, we type in the \sql\ commands, execute them by pressing~\keys{\return}, and then read their output.
