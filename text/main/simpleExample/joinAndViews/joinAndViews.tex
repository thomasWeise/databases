...
Then, we just need to write \sqlil{CREATE VIEW sale AS}\sqlIdx{CREATE!VIEW}\sqlIdx{AS} right before our new query~\cite{PGDG:PD:CV}.
...
%
\begin{sloppypar}%
For this, we simply add the clause \sqlil{WHERE (ordered >= '2025-01-01') AND (ordered < '2026-01-01')}\sqlIdx{WHERE}\sqlIdx{AND} to our queries.
