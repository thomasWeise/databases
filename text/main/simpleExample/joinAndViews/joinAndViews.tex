...
Let us create a query where, for each demand, we see the customer name, the product name, the product price, the ordered amount, and the order date.
...
\emph{\inQuotes{For each row in \sqlil{demand}, we need the corresponding row in \sqlil{customer} and the corresponding row in \sqlil{product}.}}
