...
There also are \pglspl{db} using other models, such as graphs, hierarchies, objects, and such and such~(see \cref{sec:history,sec:dbModelSelection}), but we focus on the relational approach.

Based on the data model, we must be able to define a logical schema for our \pgls{db}.
...
This logical model can be separate from the physical model of the \pgls{db}, namely how the data is actually organized on the disk.
...
In other words, at the bottom layer of the \pgls{db} is the physical model managing how data is stored.
