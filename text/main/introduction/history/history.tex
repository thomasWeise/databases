...
\begin{figure}%
\centering%
\includegraphics[width=0.3\linewidth]{\currentDir/multicsOperatorConsole}%
\floatSep%
\includegraphics[width=0.3\linewidth]{\currentDir/multicsFirstBoot}%
\floatSep%
\includegraphics[width=0.3\linewidth]{\currentDir/multicsLs}%
%
\caption{Some screenshots of the \pgls{terminal} of Multics~MR12.7 taken from~\cite{BWAOEOSH:M}, licensed under \href{https://creativecommons.org/licenses/by-sa/4.0}{CC~BY\nobreakdashes-SA~4.0}.}%
...
The hierarchical file system for the \emph{Multiplexed Information and Computing Service}~(Multics)~\pgls{OS}~\cite{CV1965IAOOTMS,BWAOEOSH:M}, published in \citeyear{DN1965AGPFSFSS}, already had surprisingly many advanced features that we know from today's file systems: fine-grained access control for data privacy, backup ability, links, and IO~queue management.
Inheriting from CTSS, it itself became the ancestor of Unix which, in turn, inspired \linux.
The \bashil{ls}~command shown in \cref{fig:BWAOEOSH:M} also was a feature of Multics (adapted from CTSS) and has survived all those years~\cite{F1999ABHOTLC}.
...
The first version of the \pgls{softIDS} was developed by \citeauthor{B2009TOOTIDSITFDAD} in 1961/62 at General Electric~\cite{B2009TOOTIDSITFDAD,B1965SFRAP}.
\Pgls{softIDS} offered the first direct access \pgls{db}, holding data in virtual memory.
...
\Pgls{softIDS} was based on a network model, further advancing data management by structuring complex relationships in data.
...
\Pgls{softIDS} and similar systems encode elements of the views of the data as part of the \pgls{db} structure, which make \pgls{softIDS} less flexible.
The Database Task Group of the Conference on Data Systems Languages~(CODASYL)~\cite{TF1976CDBMS}, a standardization body for the data processing industry, took over many of \citeauthor{B2009TOOTIDSITFDAD}'s ideas for \pgls{softIDS} in the late 1960s.
CODASYL is best known for its creation of the COBOL programming language~\cite{H2016HCBITDAFOODW}.

Only slightly after \pgls{softIDS}, another \pgls{dbms} that could handle the structured storage of records obeying datatype constraints appeared.
IBM developed the \pgls{softIMS} for the Apollo space program~\cite{KLBGNLWBS2012ITIYCGTIIMS}.
The system was launched 1965/1967~\cite{BBP2007TBOI}.
\Pgls{softIMS} is still sold as a product and still exists today.
Like \pgls{softIDS}, it was not a \pgls{dbms} for \emph{\pglspl{rdb}}.
...
The programming language offered by the \pgls{softIMS} for the data access is also relatively low-level and, for example, the search strategy has to be implemented.
Finally, changes to the logical schema will require us to perform cascading changes in the code accessing them.

Both \pgls{softIDS} and \pgls{softIMS} offered a strikingly new concept:
The application code and the code for physical data storage and retrieval were separated.
Between them, the Data Language One~(DL/I) was located as interface in \pgls{softIMS}.
\Pgls{softIDS} offered the Data Description Language~(DDL) to define types of logical records and the Data Manipulation Language~(DML) to manipulate and navigate the data.
...
Before \pgls{softIDS} and \pgls{softIMS}, this had to be part of your application's code.
...
Now this is part of \pgls{softIDS} and \pgls{softIMS}, and the user can ignore this complexity and just use a simple programming language where she can define which record to load, change, delete, or add.%
%
...
He noticed the shortcomings of the \pgls{softIDS} network model, namely that the programmers accessing the data still need a lot of information about how the data is actually represented and organized internally.
Even \citeauthor{B2009TOOTIDSITFDAD} himself mentioned that issues with \pgls{softIDS} arose when users did not consider how data is internally sorted~\cite{B2009TOOTIDSITFDAD}.
...
%
The \oracleDB, the first commercial \pgls{SQL}-based product, was released in 1979~\cite{C20245YOQ} by the company Software Development Laboratories~(SDL), which later renamed itself to Oracle~\cite{O2007OTHTMIMIOHWCFTPWMIH}.
It was an immediate success, because it was portable and could run cheaper hardware.
IBM released its \pgls{SQL} \pgls{dbms} \ibmDB2\ in 1983~\cite{C20245YOQ,HS2013THAGOID,CWDS2007UDLVWE}.
...
We will discuss them together with some commercial \pglspl{db} in the next section.%
%
\endhsection%
%
