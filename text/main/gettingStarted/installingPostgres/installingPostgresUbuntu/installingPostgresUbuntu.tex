...
%
\subfloat[][%
\textil{psql} is the client program that can connect to the \postgresql\ server and that gets installed as well. %
...
%
\subfloat[][%
In order to set a proper password for the \postgresql\ main user \textil{postgresql}, we need to log into \textil{psql} using super user privileges, but under the newly created system user \textil{postgres}. %
...
%
\subfloat[][%
Starting \textil{psql} like this connects us to the local \postgresql\ server%
...
We use the \sqlil{ALTER USER postgres PASSWORD 'XXX';} command, where \textil{XXX} is replaced with a secure password. %
...
%
\subfloat[][%
After we hit~\keys{\enter}, the system confirms the change by showing the command \sqlil{ALTER ROLE} back to us. %
From now on, the server main user has a secure password.%
...
%
\subfloat[][%
We quit \textil{psql} by typing~\keys{\textbackslash+q+\enter}.%
...
Both the client and server program are installed via \bashil{sudo apt-get install postgresql-16}, as shown in \cref{fig:installingPostgresUbuntu01aptGet}.
...
First, we need to investigate whether the \postgresql\ \pgls{DBMS} server is indeed installed and running properly.
...
We now also want to check whether the client program \bashil{psql} was installed correctly.
The client can be run in the \pgls{terminal} and allows us to communicate with the \pgls{DBMS} server.
...
The result in \cref{fig:installingPostgresUbuntu10psqlVersionB} shows that the \bashil{psql} client version on my system:~16.6.

Let us now connect to the \postgresql\ server via the client for the first time.
...
We therefore connect the \bashil{psql} client to the \postgresql\ server as superuser impersonating the \bashil{postgres} system user.
...
For the first time, we are now in the \textil{psql} client and are connected to the \postgresql\ server.
We see the prompt \expandafter\textil{postgres=\#} in \cref{fig:installingPostgresUbuntu13PsqlPrompt}.%
%
\begin{sloppypar}%
To set the password for the user (or role) \sqlil{postgres}, we type in \sqlil{ALTER USER postgres PASSWORD 'XXX';} (or \sqlil{ALTER ROLE postgres PASSWORD 'XXX';}) in \cref{fig:installingPostgresUbuntu14PsqlAlterUserPassword}.
...
\end{sloppypar}%
%
In \cref{fig:installingPostgresUbuntu15PsqlAlterUserPasswordDone}, we see that this prints \sqlil{ALTER ROLE} back to us.
...
For now, we will happily exit the \bashil{psql} client by typing~\keys{\textbackslash + q + \enter}.
