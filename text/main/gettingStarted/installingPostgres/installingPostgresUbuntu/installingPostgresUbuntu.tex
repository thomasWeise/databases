...
We can do this by typing in \bashil{systemctl status postgresql} and hit~\keys{\return}.%
...
%
\subfloat[][%
We press~\keys{q+\return} to leave the status output page.%
...
We can check its version by typing \bashil{psql --version} and hitting~\keys{\return}.%
...
We do this by writing \bashil{sudo -u postgresql psql} and hit~\keys{\return}.%
...
%
\subfloat[][%
After we hit~\keys{\return}, the system confirms the change by showing the command \sqlil{ALTER ROLE}\sqlIdx{ALTER!USER}\sqlIdx{ALTER!ROLE} back to us. %
...
%
\subfloat[][%
We quit \psql\ by typing~\keys{\textbackslash+q+\return}.%
...
When this is entered and confirmed with~\keys{\return}, the package manager checks what needs to be installed.
...
It asks us whether we are OK with that, which we confirm by typing~\keys{y+\return} in \cref{fig:installingPostgresUbuntu04yny}.
...
Running this program opens some sort of paginated mode, which we can leave by hitting~\keys{q+\return} in \cref{fig:installingPostgresUbuntu08systemctlCheckStatusQ}.%
%
\begin{sloppypar}%
...
We therefore type \bashil{psql --version} in \cref{fig:installingPostgresUbuntu09psqlVersionA}, which will print the version of this program after we hit~\keys{\return}.
...
Either way, we type \bashil{sudo -u postgres psql} into the \pgls{terminal} and hit~\keys{\return} in \cref{fig:installingPostgresUbuntu11sudoPsql}.
...
For now, we will happily exit the \psql\ \pgls{client} by typing~\keys{\textbackslash + q + \return}.
