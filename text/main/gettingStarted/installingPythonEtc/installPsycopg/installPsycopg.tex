...
Therefore, regardless whether we want to install \psycopg\ for productive use via the \pgls{terminal} or whether we instead want to use it within \pycharm\ to develop software, we will use \pglspl{virtualEnvironment}.%
%
...
We now create a directory \textil{.venv} to host a new \pgls{virtualEnvironment} by typing \bashil{mkdir .venv} and hitting~\keys{\return}.%
...
%
\subfloat[][%
We now set up a new and empty \pgls{virtualEnvironment} in this directory by writing \bashil{python3 -m venv --upgrade-deps .venv} and hitting~\keys{\return}.%
...
We now activate it by writing \bashil{source .venv/bin/activate} and hitting~\keys{\return}.%
...
%
\subfloat[][%
We install \psycopg\ into this environment by writing \bashil{pip install psycopg} and hitting~\keys{\return}.%
...
%
\subfloat[][%
Once we have finished programming, we can deactivate the \pgls{virtualEnvironment} by writing~\bashil{deactivate} and hitting~\keys{\return}. %
...
\end{figure}%
%
\Pglspl{db} are storages for large amounts of structured data.
...
We can connect to \pglspl{db} via applications and programs in order to work with the data.
Often, there are middlewares like web services or application \pglspl{server} that implement business logic and ensure that data is manipulated or evaluated in a correct way.
Such applications and programs, if developed with \python, often run in \pglspl{terminal}.
You would \emph{never} execute them within a \pgls{ide} like \pycharm.%
%
\cquotation{programmingWithPython}{The only proper way to run a \python\ application in a productive scenario is in the terminal.}%
%
Therefore, we will first explore how to install packages for this use case.
We will do so under \ubuntu\ \linux\ in the \bash\ \pgls{terminal} in \cref{fig:installPsycopgLinux}.
...
In the \bash\ shell, we can do this by typing the command \bashil{mkdir .venv} and hitting~\keys{\return} in \cref{fig:pipInstallPyscopgVenv1mkdir}
...
We set up a new and empty \pgls{virtualEnvironment} in this directory by writing \bashil{python3 -m venv --upgrade-deps .venv} and hitting~\keys{\return}.
The new \pgls{virtualEnvironment} has been created.
We now activate it by writing \bashil{source .venv/bin/activate} and hitting~\keys{\return} in \cref{fig:pipInstallPyscopgVenv3activate}.
...
In \python, we normally install packages by using the program~\pip.
\pip~automatically uses the current \pgls{virtualEnvironment}, if one is active.
We install \psycopg\ into our new environment by writing \bashil{pip install psycopg} and hitting~\keys{\return}.
...
If we run a \python\ program that uses \psycopg, like \cref{lst:factory:connect_and_select}, it will find this version of the package in our \pgls{virtualEnvironment}.
Once we have finished programming and running programs, we can deactivate the \pgls{virtualEnvironment} by writing~\bashil{deactivate} and hitting~\keys{\return}.
...
\end{figure}%
%
The \pycharm\ \pgls{ide} allows us to download all the example codes of this course and to install the \python\ packages they depend on in one step from \github.
In this process, we encounter two concepts: \git\ and the aforementioned \pglspl{virtualEnvironment}.
